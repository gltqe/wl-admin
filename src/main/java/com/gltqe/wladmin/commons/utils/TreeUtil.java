...
public class TreeUtil {
     ...
     * @date 2022/7/3 0:33
     **/
    public static <T extends TreeVo> List<T> buildTree(List<T> list, String topId) {
        Map<String, List<T>> map = list.stream().collect(Collectors.groupingBy(TreeVo::getParentId));
        for (Map.Entry<String, List<T>> entry : map.entrySet()) {
            List<T> value = entry.getValue();
            for (TreeVo treeEntity : value) {
     ...
     * @date 2022/7/3 0:34
     **/
    public static <T extends TreeVo> List<T> buildTree(List<T> list) {
    ...
    private static <T extends TreeVo> List<T> getChildren(T p, List<T> list, List<String> pids) {
